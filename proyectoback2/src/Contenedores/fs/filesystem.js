const fs = require('fs');
    ...
    async save(object){
        ...
        try{
            await fs.promises.writeFile(`./${this.nombreArchivo}`,JSON.stringify(producto,null, 2),'utf-8')
            idDinamic = object.id
            console.log(producto)
            console.log("exito: " + idDinamic)
        } catch(err){
            throw err
        }
        }
            ...
            object.id = 1
            try{
                await fs.promises.writeFile(`./${this.nombreArchivo}`,JSON.stringify([object],null, 2),'utf-8')
    ...
    async updateById(id){
        ...
        try{
            const archivo = await fs.promises.readFile(this.nombreArchivo,'utf-8');
            const productos = JSON.parse(archivo);
            const elem = await this.getById(id)
            const elemupdate = {
                nombre : elem.nombre,
                precio : elem.precio,
                img : elem.img,
